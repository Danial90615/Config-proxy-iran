		...
		try {
			if (mitmonHagdarotReshet && (Date.now() - zmanMitmonHagdarotReshet) < 30000) {
				...
				hagdarotReshet = savedNS ? JSON.parse(savedNS) : {
					...
					enableAntiSanctionDNS: true, antiSanctionDNSProvider: 'cloudflare', antiSanctionCustomDNS: '',
					...
					monthlyCapGB: 0, speedLimitKBps: 0, blockQUIC: true,
						...
						if (nativGisha === 'admin/config.json') { // Save config.json
							...
							try {
								...
								const hagdarotTkefot = {
									...
									enableAntiSanctionDNS: typeof settings.enableAntiSanctionDNS === 'boolean' ? settings.enableAntiSanctionDNS : true,
							...
							blockQUIC: typeof settings.blockQUIC === 'boolean' ? settings.blockQUIC : true,
					...
					if (bakashatMinuyLakoach || bakashatMinuyBackendHamara || keGenNivchar) {
						...
						if (userNodesTakef && userNodesTakef.trim()) {
							...
							tochenMinuy = linkTzmatimAcherim + ipNivcharMale.map((ktovetGolmit, _idx) => {
								...
								// Node remark: uses the admin's own NAMETPL (Network Settings) when set, with
									// tags {N} {NAME} {FLAG} {COUNTRY} {CITY} {ISP} {HOST} {PORT} {WORKER} {DATE} {PROTO}.
									// Falls back to the original default Nova naming only when NAMETPL is empty.
									{
										...
										const _nameNoFlag = (_flag ? _rawRemark.slice(_flag.length) : _rawRemark).trim() || ktovetTzomet;
										const _tpl = (config_JSON.NAMETPL && String(config_JSON.NAMETPL).trim())
											? String(config_JSON.NAMETPL).trim()
											: '{FLAG} سرویس رایگان نوا {HOST}:{PORT} [{PROTO}]';
										hearatTzomet = _tpl
											.replace(/\{N\}/g, String(_idx + 1))
											.replace(/\{NAME\}/g, _nameNoFlag)
											.replace(/\{FLAG\}/g, _flag)
											.replace(/\{COUNTRY\}/g, _nameNoFlag)
											.replace(/\{CITY\}/g, _nameNoFlag)
											.replace(/\{ISP\}/g, _nameNoFlag)
											.replace(/\{HOST\}/g, ktovetTzomet)
											.replace(/\{PORT\}/g, String(portTzomet))
											.replace(/\{WORKER\}/g, host || '')
											.replace(/\{DATE\}/g, new Date().toISOString().split('T')[0])
											.replace(/\{PROTO\}/g, _np.toUpperCase())
											.replace(/\s+/g, ' ')
											.trim() + _chainMark;
									}
						...
						// the raw/mixed list). Fully editable/removable from the panel's Network Settings ->
						// fakeConfigs; nothing here is force-injected or locked.
						let _fakeLines = [];
						if (bakashatMinuyLakoach) {
							...
							const _fill = (s) => String(s)
								...
								.replace('{telegram}', (hagdarotReshet && hagdarotReshet.telegramChannel) || NOVA_TG_CHANNEL);
